...
C ***********************************************************
C
      SUBROUTINE computeResNorm(Z1,Z2,Z3,N, RES_NORM)
          ! Compute norm of residual vectors
          INTEGER N
          REAL*8 Z1(N), Z2(N), Z3(N), RES_NORM
          RES_NORM = 0
          DO I=1,N
            RES_NORM = RES_NORM + Z1(I)**2 + Z2(I)**2 + Z3(I)**2
          END DO
          RES_NORM = SQRT( RES_NORM / (3*N))
      END SUBROUTINE
...
C ---  B=IDENTITY, JACOBIAN A FULL MATRIX
      DO I=1,N
         ...
         Z3(I)=Z3(I)+S3*ALPHN+S2*BETAN
      END DO
      CALL computeResNorm(Z1,Z2,Z3,N, RES_NORM)
      CALL SOL (N,LDE1,E1,Z1,IP1)
      CALL SOLC (N,LDE1,E2R,E2I,Z2,Z3,IP2)
      RETURN
C
